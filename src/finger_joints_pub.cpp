...
void publish_fing_joint_states(const sensor_msgs::JointState::ConstPtr& input_joint_msg){
	...
	whole_joint_msg.position.push_back(float (SCALE) * 1.3 * whole_joint_msg.position[0]);
	...
	whole_joint_msg.position.push_back(float (SCALE) * 1.0 * whole_joint_msg.position[0]);
